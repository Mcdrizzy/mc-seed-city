...
function frame(now){const dt=Math.min((now-previous)/1000,.05);previous=now;if(play.checked)time+=dt*20;if(spin.checked&&!drag)yaw+=dt*.3;
 ...
 groups.body.position.y=2+bob*.14;groups.body.rotation.x=flight*.035;
 groups.head.rotation.y=document.getElementById('looking').checked?Math.sin(time*.035)*.6:0;
 groups.head.rotation.x=repair?.2:0;
 groups.right_arm.rotation.set(-.06+Math.sin(time*.075)*.025,0,.035);
 ...
 if(walk){const stride=Math.sin(time*.23)*.4;groups.right_leg.rotation.x=stride;groups.left_leg.rotation.x=-stride;groups.body.rotation.x=0;groups.body.position.y=16-14*Math.cos(stride)-4*Math.abs(Math.sin(stride));groups.right_arm.rotation.x=-stride*.3;groups.left_arm.rotation.x=stride*.45;}
 groups.lantern.rotation.x=-groups.right_arm.rotation.x+Math.sin(time*.09)*.04;
 groups.wrench.rotation.z=-.15;
